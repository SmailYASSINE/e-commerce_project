...
class model
{
	...
	{
		$query=$this->db->prepare('SELECT `nom_produit`,produit.`description`,`prix`,`nom_categorie`,`image1`,`id_produit` FROM produit,categorie WHERE produit.`id_categorie`=categorie.`id_categorie`AND produit.`id_categorie`=?');
		$query->execute($A);
		return ($query->fetchAll());
	}
